...
public class PlayerActivity extends AppCompatActivity {
			...
			try {
				if (videoUri.getScheme().equals("content")) {
					...
					try {
						...
						mimeType = getContentResolver().getType(videoUri);
					} catch (Exception e) {
						if (cursor != null)
							cursor.close();
					}
				...
				Log.e("PlayerActivity", "Error processing URI: " + e.getMessage());
			}

			VideoItem externalVideo = new VideoItem("0", title, videoUri.toString(), "00:00", "External", mimeType,
		...
		try {
			if (videoList != null && !videoList.isEmpty() && position < videoList.size()) {
				...
				if (playbackService != null && playbackService.player != null) {
					...
					MediaItem mediaItem = MediaItem.fromUri(videoItem.getPath());
					...
					if (isBgPlayEnabled) {
						playbackService.startBackgroundPlay(videoItem.getTitle());
					}
				} else {
					initializePlayer();
				}
			} else {
				Toast.makeText(this, "Video load error", Toast.LENGTH_SHORT).show();
			}
		} catch (Exception e) {
	...
	private void initListeners() {
		...
		btnRotate.setOnClickListener(v -> {
			...
			setRequestedOrientation(
					orientation == Configuration.ORIENTATION_PORTRAIT ? ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE
	...
	private void setupGestures() {
		...
		gestureDetector = new GestureDetector(this, new GestureDetector.SimpleOnGestureListener() {
			...
			public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
				if (isLocked || !isTouchInsideSafeZone || e2.getPointerCount() > 1 || currentGestureMode == MODE_ZOOM)
					return false;
				...
				if (currentGestureMode == MODE_NONE) {
					if (Math.abs(totalDeltaX) > Math.abs(totalDeltaY)) {
						...
						currentGestureMode = (e1.getX() > currentWidth / 2) ? MODE_VOLUME : MODE_BRIGHTNESS;
					}
				}
		...
		playerRootContainer.setOnTouchListener((v, event) -> {
			...
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				...
				isTouchInsideSafeZone = (x > edgeInsetPx && x < (currentWidth - edgeInsetPx) && y > edgeInsetPx
						&& y < (currentHeight - edgeInsetPx));

				if (isTouchInsideSafeZone) {
		...
		for (int i = 0; i < speeds.length; i++) {
			...
			if (Math.abs(currentSpeed - values[i]) < 0.01) {
				...
				textView.setBackgroundTintList(
						android.content.res.ColorStateList.valueOf(getColor(R.color.colorAccent)));
			} else {
			...
			textView.setOnClickListener(v -> {
				if (playbackService != null && playbackService.player != null) {
					playbackService.player
							.setPlaybackParameters(new androidx.media3.common.PlaybackParameters(values[index]));
				}
				dialog.dismiss();
			});
	...
	private void showAudioBoosterDialog(boolean onLeft) {
		...
		if (audioBoosterDialog.getWindow() != null) {
			audioBoosterDialog.getWindow().setBackgroundDrawable(
					new android.graphics.drawable.ColorDrawable(android.graphics.Color.TRANSPARENT));
			WindowManager.LayoutParams params = audioBoosterDialog.getWindow().getAttributes();
			params.gravity = android.view.Gravity.CENTER_VERTICAL
					| (onLeft ? android.view.Gravity.START : android.view.Gravity.END);
			params.x = 30;
	...
	private void updateDialogUI(int volume) {
		if (boosterSeekBar != null)
			boosterSeekBar.setMax(maxUnifiedVolume);
		if (boosterSeekBar != null)
			boosterSeekBar.setProgress(volume);
		if (boosterBubbleText != null)
			boosterBubbleText.setText(String.valueOf(volume));
		if (boosterMuteIcon != null) {
			boosterMuteIcon.setImageResource(volume == 0 ? R.drawable.ic_mute : R.drawable.ic_audio_boost);
		}
	}
	...
	protected void onDestroy() {
		...
		boolean isPlaying = (playbackService != null && playbackService.player != null
				&& playbackService.player.isPlaying());
	...
	private void syncInitialBrightness() {
		try {
			int sysBright = android.provider.Settings.System.getInt(getContentResolver(),
	...
	private void setBrightness(int value) {
		...
		lp.screenBrightness = (value <= 0) ? 0.01f : value / 100.0f;
		...
		if (brightnessText != null)
			brightnessText.setText(String.valueOf(value));
	}
	...
	private void showBrightnessDialog() {
		...
		if (brightnessDialog.getWindow() != null) {
			brightnessDialog.getWindow().setBackgroundDrawable(
					new android.graphics.drawable.ColorDrawable(android.graphics.Color.TRANSPARENT));
	...
	private void enableFullScreenMode() {
		...
		// Android R (API 30) and above - Modern method
		...
		// Android 10 and below - Legacy method
		else {
			View decorView = getWindow().getDecorView();

			// Notch support for Android 9+
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
				getWindow()
						.getAttributes().layoutInDisplayCutoutMode = WindowManager.LayoutParams.LAYOUT_IN_DISPLAY_CUTOUT_MODE_SHORT_EDGES;
			}
			...
			// Re-hide system bars when they reappear
			decorView.setOnSystemUiVisibilityChangeListener(new View.OnSystemUiVisibilityChangeListener() {
				@Override
				public void onSystemUiVisibilityChange(int visibility) {
					if ((visibility & View.SYSTEM_UI_FLAG_FULLSCREEN) == 0) {
						// System bars visible, hide them again after delay
		...
		// Transparent system bars (optional - for edge-to-edge)
